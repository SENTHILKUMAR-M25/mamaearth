...
const CategoryControl = () => {
    ...
    const handleSubmit = async () => {
        if (!form.name || !form.disc) return alert("Please fill all fields");
        
        setLoading(true);
        try {
            const formData = new FormData();
            formData.append("name", form.name);
            formData.append("disc", form.disc); // Ensure 'disc' matches backend schema
    ...
    // ✏️ Edit - FIXED: Now maps 'disc' correctly
    const handleEdit = (cat) => {
        setForm({
            name: cat.name,
            disc: cat.disc || "", // FIX: Populate description in modal
            image: null,
            preview: cat.image
                                ...
                                <tr key={c._id} className="border-t hover:bg-gray-50 transition">
                                    ...
                                    <td className="py-2">
                                        {c.image ? (
                                            ...
                                            <div className="h-12 w-12 bg-gray-100 rounded flex items-center justify-center text-xs text-gray-400">No Image</div>
                                        )}
                                    </td>
                                    <td className="font-semibold text-gray-700">{c.name}</td>
                                    <td className="text-gray-600 text-sm max-w-xs truncate">{c.disc}</td>
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex justify-center items-center z-50 p-4">
                    <div className="bg-white p-6 rounded-3xl w-full max-w-md shadow-2xl">
                        ...
                        <div className="flex justify-end gap-3 mt-8">
                            ...
                            >
                                {loading ? "Processing..." : (editId ? "Save Changes" : "Create Category")}
                            </button>
                        </div>
                    </div>
                </div>
